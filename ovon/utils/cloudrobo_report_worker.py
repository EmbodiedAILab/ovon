...
class CloudRoboReportWorkerProcess(ProcessBase):
    ...
    episodes_info: Dict[str, Dict[str, int]] = attr.ib(
        init=False, factory=lambda: defaultdict(dict)
    )
    episodes_info_sum: Dict[str, int] = attr.ib(
        init=False, factory=lambda: defaultdict(int)
    )
    episode_finish_flag: bool = False

    def get_env_status(self):
        self.response_queue.put(self.episode_finish_flag)
    ...
    def statistics_episodes(self, data):
        ...
        tmp_episode_val = {}
        tmp_episode_val["max_episodes"] = int(data["episodes"]["max_episodes"])
        tmp_episode_val["episodes_completed"] = int(data["episodes"]["episodes_completed"])
        tmp_episode_val["scenes_used"] = int(data["episodes"]["scenes_used"])
        tmp_episode_val["cycle_finish"] = bool(data["episodes"]["cycle_finish"])

        self.episodes_info[tmp_episode_key] = tmp_episode_val
        tmp_sum_val = {"max_episodes": 0, "episodes_completed": 0, "scenes_used": 0, "cycle_finish": True}
        for ep_val in list(self.episodes_info.values()):
            for k in ep_val.keys():
                if k == "cycle_finish":
                    tmp_sum_val[str(k)] = tmp_sum_val[str(k)] and ep_val[str(k)]
                else:
                    tmp_sum_val[str(k)] += ep_val[str(k)]
            
        self.episodes_info_sum = tmp_sum_val
    ...
    def log_metrics(
        ...
        all_episodes_process = gather_objects(
            dict(self.episodes_info_sum), device=self.device
        )
        ...
        if rank0_only():
            ...
            all_episodes_process_sum = {"max_episodes": 0, "episodes_completed": 0, "scenes_used": 0, "cycle_finish": True}
            for item in all_episodes_process:
                if item is None or item == {}:
                    continue
                all_episodes_process_sum['max_episodes'] += item['max_episodes']
                all_episodes_process_sum['episodes_completed'] += item['episodes_completed']
                all_episodes_process_sum['scenes_used'] += item['scenes_used']
                all_episodes_process_sum['cycle_finish'] = all_episodes_process_sum['cycle_finish'] and item['cycle_finish']
            if all_episodes_process_sum['cycle_finish']:
                self.episode_finish_flag = True
            ...
            for k in self.window_episode_stats.keys():
                ...
                writer.add_scalar(
                    f"metrics_ep/{k}", self.window_episode_stats[k].mean , all_episodes_process_sum['episodes_completed']
                )
...
class CloudRoboReportWorker(WorkerBase):
    def __init__(
        self,
        mp_ctx: BaseContext,
        port: int,
